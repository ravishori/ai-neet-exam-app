"""READ-ONLY ECAEP submission dry-run for Biology Ch1 batch.
...
Usage:
  python scripts/ecaep_bio_ch1_submission_dry_run.py
"""
...
def is_batch_item(item: ContentItem) -> bool:
    ...
    if BATCH in tags or any(BATCH in (t or "") for t in tags):
        return True
    return bool(item.slug and "bio11-ch01-b001" in item.slug.lower())
...
async def main() -> int:
    async with AsyncSessionLocal() as session:
        ...
        non_draft_activeish = [
            i for i in batch_all if i.status in {"PUBLISHED", "APPROVED", "IN_REVIEW"}
        ]
        other_batch_leak = []
        for i in all_items:
            if i.status != "DRAFT":
            ...
            if is_batch_item(i):
                continue
            # detect accidental phy/other inclusion candidates (should be 0 in selection)
        ...
        # Prove chapter/subject via concept taxonomy (canonical Botany / Living World)
        ...
        for item in selection:
            ...
            # Supplemental confirmation (not a parallel gate): concept row + path
            taxonomy_confirm = {
            ...
            # Provenance presence (informational; not a submit_for_review gate)
        ...
        selection_proof = {
            ...
            "proofs": {
                "superseded_excluded": len(superseded) == 5
                and all(i.status == "SUPERSEDED" for i in superseded),
                ...
                "all_botany_living_world": len(wrong_chapter) == 0,
            },
        }
        ...
        ok = (
            ...
            and selection_proof["proofs"]["all_botany_living_world"]
        ...
        lines = [
            ...
            f"- Non–Botany/Living-World taxonomy: **{len(wrong_chapter)}**",
            "",
            "```json",
            ...
            "",
            "Order: numeric Q000001–Q000098, then R000095–R000100 "
            "(stable `sort_key` on external id suffix).",
            "",
        ...
        lines += [
            ...
            "```",
            "",
            "## 5. Database safety (read-only snap)",
            "",
            "```json",

"""PYTHON-MCQ-ENGINE-013 — high-yield non-definitional reviewed expansion.

Reviews up to 200 NEW ENGINE-011 candidates using ENGINE-012-viable types only.
Defers RELATIONSHIP_FORMULA. Creates a NEW fixture.
Does not modify ENGINE-006/010/011/012. No MCQ persistence. No provider calls.
"""
...
PACK_OUT = BACKEND / "tests/fixtures/python_mcq_engine_013_reviewed_nondef_v1.json"
AUDIT_JSON = ROOT / "docs/audits/python_mcq_engine_013.json"
AUDIT_MD = ROOT / "docs/audits/python_mcq_engine_013.md"
SYLLABUS = ROOT / "NEETSyllabus.txt"
REVIEWED_AT = datetime(2026, 9, 14, 14, 30, tzinfo=UTC)
RETIRED_FACT_ID = (
    "ncert-fact-v1-bbe13bb91d3757573671d3531c1912cd7c7c92e825153cf6400470f1f625fa99"
)

# RELATIONSHIP_FORMULA deferred (ENGINE-012: 0/25 eligible).
TARGET_MIX = {
    "DIRECT_FACT": 100,
    "CONTROLLED_ASSOCIATION": 60,
    "CONTROLLED_NUMERICAL": 25,
    "SI_UNIT_DIMENSION": 15,
}
DEFERRED_CATEGORIES = frozenset({"RELATIONSHIP_FORMULA"})
SUBJECT_PRIORITY = ("ZOOLOGY", "PHYSICS", "BOTANY", "CHEMISTRY")
DISTRACTOR_FAILURE_REASONS = frozenset(
    {"DISTRACTOR_POOL_INSUFFICIENT", "DISTRACTOR_EVIDENCE_MISSING"}
)
...
def _select_candidates(
    ...
    *,
    ...
    """Select up to target mix from unused ENGINE-011 candidates.

    Prefers Zoology, then diversifies across subjects. Reports shortfalls
    when fewer suitable candidates exist than the suggested quota.
    """
    ...
    for item in indexed:
        ...
        if category in DEFERRED_CATEGORIES or category not in TARGET_MIX:
            continue
        fact = item["fact"]
        if fact["fact_id"] in excluded_ids:
            continue
        if fact["fact_id"] == RETIRED_FACT_ID:
            continue
        canon = _norm_key(fact.get("canonical_fact") or fact["evidence_text"])
        if canon in excluded_canonicals:
            continue
        pools[category][fact["subject"]].append(item)
        available_counts[category][fact["subject"]] += 1
    ...
    for category, quota in TARGET_MIX.items():
        ...
        available_total = sum(len(items) for items in by_subject.values())
        effective_quota = min(quota, available_total)
        ...
        if available_subjects:
            base = max(1, effective_quota // len(available_subjects))
            for subject in available_subjects:
                share = base + (2 if subject == "ZOOLOGY" else 0)
                take(subject, share)
        for subject in SUBJECT_PRIORITY:
            if taken_for_category >= effective_quota:
                break
            take(subject, effective_quota - taken_for_category)

        shortfall[category] = {
            "target": quota,
            "available_unused": available_total,
            "selected": taken_for_category,
            "shortfall": max(0, quota - taken_for_category),
            "by_subject_available": dict(available_counts.get(category, {})),
        }

    return selected, shortfall


def _classify_engine012_distractor_failures(
    pack012: dict[str, Any],
    candidates: dict[str, Any],
) -> dict[str, Any]:
    """Classify ENGINE-012's distractor failures without recovering them."""
    facts = {fact["fact_id"]: fact for fact in candidates["pack"]["facts"]}
    rows: list[dict[str, Any]] = []
    class_counts: Counter[str] = Counter()
    for disposition in pack012.get("review_dispositions", []):
        reason = disposition.get("failure_reason") or ""
        if reason not in DISTRACTOR_FAILURE_REASONS:
            continue
        fact = facts.get(disposition["fact_id"], {})
        evidence = _norm(fact.get("evidence_text") or "")
        category = disposition.get("analysis_category")
        classification = "distractor_construction_failure"
        detail = reason
        if category == "RELATIONSHIP_FORMULA":
            if _OCR_JUNK.search(evidence) or "Eq" in evidence or "´" in evidence:
                classification = "ocr_evidence_quality_problem"
            elif _extract_formula(evidence) is None:
                classification = "ocr_evidence_quality_problem"
            else:
                classification = "distractor_construction_failure"
            detail = f"{reason}; RELATIONSHIP_FORMULA deferred by ENGINE-013"
        elif category == "CONTROLLED_NUMERICAL":
            if _QUESTION_START.search(evidence) or "you have studied" in evidence.casefold():
                classification = "underlying_fact_invalid"
            elif _RANGE_NUM.search(evidence) or _extract_numerical(evidence) is None:
                classification = "insufficient_ncert_evidence"
            else:
                classification = "distractor_construction_failure"
        elif category == "SI_UNIT_DIMENSION":
            if _extract_si(evidence) is None:
                classification = "insufficient_ncert_evidence"
            elif "W)" in evidence or "m–2" in evidence:
                classification = "ocr_evidence_quality_problem"
            else:
                classification = "distractor_construction_failure"
        else:
        ...
        rows.append(
            {
                "fact_id": disposition["fact_id"],
                "analysis_category": category,
                "subject": disposition.get("subject"),
                "chapter": disposition.get("chapter"),
                "failure_reason": reason,
                "classification": classification,
                "detail": detail,
                "evidence_excerpt": evidence[:160],
            }
        )
    return {
        "count": len(rows),
        ...
        "cases": rows,
        "note": (
            "ENGINE-013 does not weaken distractor safety to recover these "
            "candidates. RELATIONSHIP_FORMULA remains deferred."
        ),
    }
        ...
        **base,
        ...
        "provenance": {
            **base.get("provenance", {}),
            ...
            "notes": (
                f"ENGINE-013 independently reviewed non-definitional candidate "
    ...
    return {
        ...
        "provenance": {
            **(fact.get("provenance") or {}),
            "extracted_by": TASK_ID,
            "source_audit": "docs/audits/python_mcq_engine_012.json",
            "notes": f"ENGINE-013 disposition={status}; reason={reason}",
        },
        "review_record": {
            "reviewed_by": TASK_ID,
            "reviewed_at": REVIEWED_AT.isoformat(),
...
def main() -> int:
    ...
    pack012_bytes = PACK_012.read_bytes()
    ...
    pack012 = json.loads(pack012_bytes)
    existing_ids = {fact["fact_id"] for fact in pack010["facts"]}
    existing_ids |= {fact["fact_id"] for fact in pack012["pack"]["facts"]}
    existing_ids |= {
        disposition["fact_id"] for disposition in pack012["review_dispositions"]
    }
    existing_canonical = {
        _norm_key(fact["canonical_fact"]) for fact in pack010["facts"]
    }
    existing_canonical |= {
        _norm_key(fact["canonical_fact"]) for fact in pack012["pack"]["facts"]
    }

    distractor_analysis = _classify_engine012_distractor_failures(
        pack012, candidate_payload
    )
    ...
    selected, selection_shortfall = _select_candidates(
        indexed,
        excluded_ids=existing_ids,
        excluded_canonicals=existing_canonical,
    )
    ...
    for item in selected:
        ...
        if fact["fact_id"] in existing_ids or _norm_key(
            ...
            row = _reject_record(item, "DUPLICATE_PRIOR_CORPUS")
            reviewed_rows.append(row)
            status_counts["REJECTED"] += 1
    ...
    out_payload = {
        "pack": {
            "schema_version": FACT_PACK_SCHEMA_VERSION,
            "pack_id": "python-mcq-engine-013-reviewed-nondef-v1",
        ...
        "selection": {
            ...
            "shortfall": selection_shortfall,
            "deferred_categories": sorted(DEFERRED_CATEGORIES),
            "source_fixture": str(CANDIDATES),
            "excluded_prior_reviewed_ids": len(existing_ids),
        },
        "engine012_distractor_failure_analysis": distractor_analysis,
        "note": (
            "ENGINE-013 independently reviewed unused ENGINE-011 candidates using "
            "ENGINE-012-viable fact types only (RELATIONSHIP_FORMULA deferred). "
            "Only facts passing every gate are REVIEWED and included in pack.facts "
            "as MCQ_ELIGIBLE. No automatic promotion. ENGINE-006/010/011/012 unchanged."
        ),
    }
    ...
    selected_count = len(selected)
    eligibility_rate = (
        round(eligible_count / selected_count, 4) if selected_count else 0.0
    )
    ...
    audit = {
        ...
        "verdict": "GREEN",
        "baseline": {
            "engine_010_mcq_eligible": 414,
            "engine_012_mcq_eligible": 58,
            "combined_potential_before_013": 472,
        },
        "candidates_selected": selected_count,
        "candidates_reviewed": selected_count,
        ...
        "duplicate_count": int(
            ...
            + reason_codes.get("DUPLICATE_PRIOR_CORPUS", 0)
        ),
        "ambiguity_count": int(
        ...
        "subject_breakdown": {
            ...
            "eligible": dict(Counter(fact["subject"] for fact in clean_eligible)),
        },
        "fact_type_breakdown": {
        ...
        "fixture_mutations": {
            ...
            "engine_012_modified": PACK_012.read_bytes() != pack012_bytes,
            "engine_013_created": True,
            "engine_010_eligible_count_unchanged": len(pack010["facts"]) == 414,
            "engine_012_eligible_count_unchanged": len(pack012["pack"]["facts"]) == 58,
        },
        ...
        "combined_eligible_after_013_if_merged": 414 + 58 + eligible_count,
    }
    ...
    eligible_by_cat = audit["fact_type_breakdown"]["eligible_by_category"]
    lines = [
        "# PYTHON-MCQ-ENGINE-013 — High-Yield Non-definitional Expansion",
        "",
        f"**Verdict:** **{audit['verdict']}**",
        f"**Candidates selected / reviewed:** **{selected_count} / {selected_count}**",
        f"**MCQ_ELIGIBLE:** **{eligible_count}**",
        f"**REVIEW_REQUIRED:** **{review_required_count}**",
        f"**REJECTED:** **{rejected_count}**",
        f"**Eligibility rate:** **{eligibility_rate}**",
        "",
        "## Baseline",
        "",
        "- ENGINE-010: **414**",
        "- ENGINE-012: **58**",
        f"- ENGINE-013 new eligible: **{eligible_count}**",
        f"- Combined if merged: **{414 + 58 + eligible_count}**",
        "",
    ...
    for category, quota in TARGET_MIX.items():
        info = selection_shortfall.get(category, {})
        lines.append(
            "| {cat} | {target} | {avail} | {sel} | {short} | {elig} |".format(
                cat=category,
                target=quota,
                avail=info.get("available_unused", 0),
                sel=info.get("selected", 0),
                short=info.get("shortfall", 0),
                elig=eligible_by_cat.get(category, 0),
            )
        )
    lines.extend(
        [
            ...
            "",
            f"- Count: **{distractor_analysis['count']}**",
            f"- Classifications: `{distractor_analysis['classification_counts']}`",
            f"- {distractor_analysis['note']}",
            "",
        ...
        json.dumps(
            {
                ...
                "eligibility_rate": eligibility_rate,
                "shortfall": {
                    category: info["shortfall"]
                    for category, info in selection_shortfall.items()
                },
                "subjects_selected": dict(subject_counts),
                "distractor_analysis": distractor_analysis["classification_counts"],

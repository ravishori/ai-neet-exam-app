"""Atomic NCERT SOURCE_TEXT_VERIFIED certification for Biology Ch2 batch.

Uses ContentWorkflowService.certify_ncert_evidence(..., commit=False)
then a single session.commit(). Never publishes.

Usage (from apps/backend):
  python scripts/execute_bio_ch2_ncert_certification.py            # dry-run default
  python scripts/execute_bio_ch2_ncert_certification.py --commit
"""
...
EXPECTED_CH02_POST = EXPECTED_CH02_PRE
...
NCERT_PDF = (
    ...
    / "Biology"
    / "Class 11-Biology"
    / "ncert-books-class-11-biology-chapter-2.pdf"
)
EXPECTED_NCERT_SHA = "023adc154e4ba8edb5b472e3630065f7b728ba0002960c15da48c3c4ff102e3f"
AUTH_REPAIRED_SHA = "020d48816b5c318928cf2edda28703e17e923929a262b86be93b387b4f41637c"
POST_REPAIR = ROOT / "post_repair_integrity_audit.json"
...
CH02_CERT_METHOD = (
    ...
    "batch_audit+ch2_pdf;level=SOURCE_TEXT_VERIFIED;no_page_invented"
)
...
def planned_ids_from_live(by_eid: dict[str, ContentItem]) -> list[str]:
    return sorted(eid for eid, i in by_eid.items() if i.status == "APPROVED")
...
async def preflight(session) -> dict:
    ...
    repaired_sha = sha_file(ROOT / "questions_repaired.jsonl")
    ...
    if REPL not in planned:
        raise Abort("R000085_missing_from_plan", REPL, planned[-5:])
    if ORIG in planned:
        raise Abort("original_Q000085_in_plan", "excluded", ORIG)
    if superseded_live != [ORIG]:
        raise Abort("superseded_set", [ORIG], superseded_live)

    if POST_REPAIR.exists():
        audit = json.loads(POST_REPAIR.read_text(encoding="utf-8"))
        expected_active = set(audit.get("active_set", {}).get("active_external_ids") or [])
        if expected_active and set(planned) != expected_active:
            raise Abort(
                "active_set_mismatch_vs_post_repair_audit",
                sorted(expected_active - set(planned))[:5],
                sorted(set(planned) - expected_active)[:5],
            )

    # R000085 focused stem check
    repl = by_eid[REPL]
    repl_ver = latest_version(repl)
    repl_body = dict(repl_ver.body or {}) if repl_ver else {}
    ...
    r000085_check = {
        ...
        "will_certify": True,
        "original_excluded": ORIG not in planned and by_eid[ORIG].status == "SUPERSEDED",
    }
    if not r000085_check["stem_has_partially_heterotrophic"]:
        raise Abort("R000085_stem_missing_heterotrophic", True, stem)
    if r000085_check["stem_has_partially_autotrophic"]:
        raise Abort("R000085_stem_still_autotrophic", False, stem)
    ...
    # Superseded must be ineligible
    orig_gate = await workflow.evaluate_certify_ncert(by_eid[ORIG].id, required_batch_id=BATCH)
    if orig_gate["eligible"]:
        raise Abort("superseded_eligible", False, True)
    ...
    return {
        ...
        "r000085_check": r000085_check,
        "orig_gate_ineligible": {
            "eligible": orig_gate["eligible"],
            "rejection_code": orig_gate["rejection_code"],
        },
    }
...
async def post_verify(session, pf: dict) -> dict:
    ...
    for eid in pf["superseded_excluded"]:
        ...
        if not item or item.status != "SUPERSEDED":
            unexpected.append(f"superseded_status:{eid}")
        else:
            ver = latest_version(item)
            body = dict(ver.body or {}) if ver else {}
            if ncert_level(body) == NCERT_CERTIFICATION_LEVEL:
                unexpected.append(f"superseded_certified:{eid}")

    # R000085 still correct stem
    repl = by_eid.get(REPL)
    if repl:
        body = dict(latest_version(repl).body or {})
    ...
    if post["student_ch02"] != 0 or post["practice_nonpub_ch02"] != 0:
...
async def publication_gate_batch(session, pf: dict) -> dict:
    ...
    would_publish = 0
    ncert_blocker = 0
    samples = [pf["planned_ids"][0], REPL, pf["planned_ids"][-1]]
    sample_results = []
...
async def idempotency_probe(session, sample_item_id: uuid.UUID) -> dict:
    ...
    result = await workflow.certify_ncert_evidence(
        ...
        verification_method=CH02_CERT_METHOD,
...
def write_report(payload: dict) -> None:
    ...
    lines = [
        ...
        json.dumps(
            {
                "ch02": (payload.get("post_state") or {}).get("ch02"),
                "verification_levels": payload.get("verification_levels"),
                "student_ch02": (payload.get("post_state") or {}).get("student_ch02"),
                "practice_nonpub_ch02": (payload.get("post_state") or {}).get("practice_nonpub_ch02"),
                "ch01_PUBLISHED": (payload.get("post_state") or {}).get("ch01", {}).get("PUBLISHED"),
                "physics_DRAFT": payload.get("physics_DRAFT"),
            },
            indent=2,
        ),
        "```",
        "",
        f"- Certified count: **{payload.get('certified_count')}**",
...
def write_stage(payload: dict) -> None:
    ...
    stage = {
        ...
        "verdict": "GREEN — BIO11-CH02-B001 NCERT CERTIFICATION COMPLETE",
        ...
        "counts": (payload.get("post_state") or {}).get("ch02"),
        "verification_levels": payload.get("verification_levels"),
        "stages": {
        ...
        "artifacts": {"ncert_verification_execution_report": str(OUT_JSON)},
    }
    STAGE_JSON.write_text(json.dumps(stage, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    STAGE_MD.write_text(
        f"""# Pipeline stage status — `{BATCH}`

## GREEN — BIO11-CH02-B001 NCERT CERTIFICATION COMPLETE
...
async def run(*, do_commit: bool) -> dict:
    ...
    commands = [
        f"python scripts/execute_bio_ch2_ncert_certification.py{' --commit' if do_commit else ''}",
    ]
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not do_commit:
            ...
            payload = {
                ...
                "r000085_check": pf["r000085_check"],
                "pre_state": pf["pre"],
                "post_state": None,
        ...
        gates = await publication_gate_batch(session, pf)
        sample_id = uuid.UUID(pf["fingerprints"][REPL]["content_item_id"])
        ...
        green = (
            ...
            and idem["ok"]
            and tests["ok"]
            and post["post"]["ch02"]["PUBLISHED"] == 0
            and post["post"]["student_ch02"] == 0
            and changed_count == 100
            and REPL in pf["planned_ids"]
            and ORIG in pf["superseded_excluded"]
        )
        remaining = {
            k: v
        ...
        payload = {
            ...
            "r000085_check": {
                **pf["r000085_check"],
                "certified": any(
                    c["question_id"] == REPL and c["changed"] for c in certified
                ),
            },
            "pre_state": pf["pre"],
            ...
            "student_visibility": post["post"]["student_ch02"],
            "practice_nonpub_hits": post["post"]["practice_nonpub_ch02"],
            ...
            "architecture": {
                ...
                "cli": "scripts/execute_bio_ch2_ncert_certification.py",
            ...
            "assertions": {
                ...
                "APPROVED_100": post["post"]["ch02"]["APPROVED"] == 100,
                "PUBLISHED_0": post["post"]["ch02"]["PUBLISHED"] == 0,
                "SUPERSEDED_1": post["post"]["ch02"]["SUPERSEDED"] == 1,
                "student_visible_0": post["post"]["student_ch02"] == 0,
                "practice_pool_0": post["post"]["practice_nonpub_ch02"] == 0,
                "R000085_certified": True,
                "Q000085_not_certified": True,
                "publication_not_executed": True,
                "idempotency_ok": idem["ok"],
                "tests_ok": tests["ok"],
                "no_unexpected_mutations": len(post["unexpected_mutations"]) == 0,
            },
            "failure": None if green else {"unexpected_mutations": post["unexpected_mutations"], "tests": tests},
            "explicit_stop": [
                "PUBLICATION NOT EXECUTED.",
                "MANDATORY STOP REACHED.",
            ],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Biology Ch2 NCERT certification")

"""Execute atomic ECAEP submit_for_review for Biology Ch3 100 DRAFTs.
...
Before real commit, runs an induced-failure atomicity test (3 submits + rollback).

Usage:
  python scripts/execute_bio_ch3_ecaep_submission.py --commit
  python scripts/execute_bio_ch3_ecaep_submission.py --dry-preflight
"""
...
EXPECTED_BIO_PRE = {"DRAFT": 100, "SUPERSEDED": 0, "PUBLISHED": 0, "APPROVED": 0, "IN_REVIEW": 0}
EXPECTED_BIO_POST = {"DRAFT": 0, "SUPERSEDED": 0, "PUBLISHED": 0, "APPROVED": 0, "IN_REVIEW": 100}
...
def assert_ch01_regression(snap_data: dict) -> None:
    ...
    if snap_data.get("ch02_regression") != EXPECTED_CH02:
        raise Abort("ch02_regression", EXPECTED_CH02, snap_data.get("ch02_regression"))
...
async def preflight(session, plan: dict) -> dict:
    ...
    superseded_live = [eid for eid, i in by_eid.items() if i.status == "SUPERSEDED"]
...
async def atomicity_rollback_test(session, pf: dict) -> dict:
    """Submit first N with commit=False, induce failure, rollback — expect DRAFT=100."""
    workflow = ContentWorkflowService(session)
    ...
    ok = (
        induced
        and restored["biology"] == EXPECTED_BIO_PRE
        and restored["biology"]["IN_REVIEW"] == 0
        and restored["biology"]["DRAFT"] == 100
    )
    assert_ch01_regression(restored)
    return {
        "induced_failure": induced,
        "partial_submit_count": len(partial),
        "partial_question_ids": partial,
        "restored_biology": restored["biology"],
        "restored_to_pre": restored["biology"] == EXPECTED_BIO_PRE,
...
async def post_verify(session, preflight_data: dict) -> dict:
    ...
    if post["ch01_regression"] != EXPECTED_CH01:
        unexpected_mutations.append(f"ch01_regression={post['ch01_regression']}")
...
def write_report(payload: dict) -> None:
    ...
    lines = [
        ...
        "",
        "```json",
        json.dumps(payload.get("atomicity_rollback_test"), indent=2),
        "```",
        "",
        "## Assertions",
        "",
...
async def run(*, do_commit: bool) -> dict:
    ...
    async with AsyncSessionLocal() as session:
        ...
        # Induced failure atomicity test (must pass before real commit)
        try:
        ...
        if not rb_test["ok"]:
            payload = {
                ...
                "failure": {
                    "condition": "atomicity_rollback_test",
                    "expected": EXPECTED_BIO_PRE,
                    "actual": rb_test.get("restored_biology"),
                },
                "pre_state": pf["pre"],
            }
            write_report(payload)
...
async def main() -> int:
        ...
        json.dumps(
            {
                ...
                "atomicity_ok": (result.get("atomicity_rollback_test") or {}).get("ok"),
    ...
    preflight_only = result.get("transaction_result") == "NO_COMMIT" and result.get("submitted_count") == 0
    if preflight_only and "PREFLIGHT READY" in str(result.get("verdict", "")):
        return 0

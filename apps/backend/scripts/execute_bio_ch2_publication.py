"""AUTHORIZED controlled publication for Biology Ch2 (BIO11-CH02-B001).

Uses ONLY ContentWorkflowService.publish(item_id) per plan ID.
Does NOT use bulk publish API.
Stops immediately on first failure (non-atomic — same as CH01).

Usage (from apps/backend):
  python scripts/execute_bio_ch2_publication.py            # preflight-only
  python scripts/execute_bio_ch2_publication.py --commit   # authorized publish
"""
...
EXPECTED_CH02_TAX = {"chapters": 1, "topics": 6, "bc_concepts": 9}
EXPECTED_CH02_PRE = {
    "DRAFT": 0,
    "SUPERSEDED": 1,
    "PUBLISHED": 0,
...
AUTH_REPAIRED_SHA = "020d48816b5c318928cf2edda28703e17e923929a262b86be93b387b4f41637c"
...
async def snap(session) -> dict:
    ...
    ch2 = [i for i in items if is_batch_item(i, BATCH, "bio11-ch02-b001")]
    ch1 = [i for i in items if is_batch_item(i, CH01, "bio11-ch01-b001")]
    phy = [i for i in items if any(PHY in (t or "") for t in (i.tags or []))]
    bs = Counter(i.status for i in ch2)
    ch1s = Counter(i.status for i in ch1)
    repo = CmsRepository(session)
    student_ch2 = student_ch1 = 0
    off = 0
    while True:
        page, tot = await repo.list_questions(class_level="11", limit=100, offset=off)
        student_ch2 += sum(1 for i in page if is_batch_item(i, BATCH, "bio11-ch02-b001"))
        student_ch1 += sum(1 for i in page if is_batch_item(i, CH01, "bio11-ch01-b001"))
        off += 100
        if off >= tot or not page:
            break
    pool = set(await AssessmentRepository(session).published_question_ids_for_scope("FULL", None))
    ch2_pub = {i.id for i in ch2 if i.status == "PUBLISHED"}
    ch2_nonpub = {i.id for i in ch2 if i.status != "PUBLISHED"}
    return {
        "taxonomy": {
            "subjects": tax[0],
            "chapters": tax[1],
        ...
        "student_ch02": student_ch2,
        "student_ch01": student_ch1,
        "practice_ch02": len(ch2_pub & pool),
        "practice_nonpub_ch02": len(ch2_nonpub & pool),
    }
...
async def load_batch_map(session) -> dict[str, ContentItem]:
    ...
    for i in items:
        if not is_batch_item(i, BATCH, "bio11-ch02-b001"):
            continue
...
async def build_plan(session) -> list[dict]:
    ...
    planned = sorted(eid for eid, i in by_eid.items() if i.status == "APPROVED")
    actions = []
    for eid in planned:
        item = by_eid[eid]
        ver = latest_version(item)
...
async def preflight(session, plan_actions: list[dict]) -> dict:
    repaired = sha_file(ROOT / "questions_repaired.jsonl")
    if repaired != AUTH_REPAIRED_SHA:
        raise Abort("questions_repaired_sha", AUTH_REPAIRED_SHA, repaired)
    ...
    if REPL not in qids:
        raise Abort("R000085_missing", REPL, None)
    if ORIG in qids:
        raise Abort("Q000085_in_plan", "excluded", ORIG)

    pre = await snap(session)
    if pre["taxonomy"] != EXPECTED_TAX:
        raise Abort("taxonomy", EXPECTED_TAX, pre["taxonomy"])
    if pre["ch02_taxonomy"] != EXPECTED_CH02_TAX:
        raise Abort("ch02_taxonomy", EXPECTED_CH02_TAX, pre["ch02_taxonomy"])
    if pre["ch02"] != EXPECTED_CH02_PRE:
        raise Abort("ch02_pre", EXPECTED_CH02_PRE, pre["ch02"])
    if pre["ch01"].get("PUBLISHED") != 100:
        raise Abort("ch01_published", 100, pre["ch01"])
    if pre["physics_DRAFT"] != 24:
        raise Abort("physics_DRAFT", 24, pre["physics_DRAFT"])
    if pre["student_ch02"] != 0 or pre["practice_nonpub_ch02"] != 0:
        raise Abort("student_safety", 0, pre)

    by_eid = await load_batch_map(session)
    superseded_live = sorted(eid for eid, i in by_eid.items() if i.status == "SUPERSEDED")
    if superseded_live != [ORIG]:
        raise Abort("superseded_set", [ORIG], superseded_live)

    # R000085 stem
    repl = by_eid[REPL]
    repl_body = dict(latest_version(repl).body or {})
    stem = repl_body.get("stem") or ""
    r000085 = {
        "external_id": REPL,
        "status": repl.status,
        "ncert": ncert_level(repl_body),
        "stem_has_partially_heterotrophic": "partially heterotrophic" in stem,
        "stem_has_partially_autotrophic": "partially autotrophic" in stem,
        "in_plan": True,
    }
    if not r000085["stem_has_partially_heterotrophic"] or r000085["stem_has_partially_autotrophic"]:
        raise Abort("R000085_stem", "partially heterotrophic", stem)
    if r000085["ncert"] != "SOURCE_TEXT_VERIFIED":
        raise Abort("R000085_ncert", "SOURCE_TEXT_VERIFIED", r000085["ncert"])
    ...
    # Superseded must fail publish gate / not be in plan
    orig = by_eid[ORIG]
    if orig.status != "SUPERSEDED":
        raise Abort("orig_not_superseded", "SUPERSEDED", orig.status)
    ...
    return {
        ...
        "ineligible": 0,
        "r000085": r000085,
        "questions_repaired_sha256": repaired,
    }
...
async def post_audit(session, pf: dict) -> dict:
    ...
    if not repl or repl.status != "PUBLISHED":
        unexpected.append("R000085_not_published")
    else:
        stem = (dict(latest_version(repl).body or {}).get("stem") or "")
        if "partially heterotrophic" not in stem or "partially autotrophic" in stem:
            unexpected.append("R000085_stem_regression")
    ...
    if post["ch02"] != EXPECTED_CH02_POST:
        unexpected.append(f"ch02_post={post['ch02']}")
    if post["taxonomy"] != EXPECTED_TAX:
        unexpected.append(f"taxonomy={post['taxonomy']}")
    if post["ch02_taxonomy"] != EXPECTED_CH02_TAX:
        unexpected.append(f"ch02_taxonomy={post['ch02_taxonomy']}")
    ...
    if post["student_ch02"] != 100:
        unexpected.append(f"student_ch02={post['student_ch02']}")
    if post["practice_ch02"] != 100:
        unexpected.append(f"practice_ch02={post['practice_ch02']}")
...
async def verify_student_and_practice(session, pf: dict) -> dict:
    ...
    by_eid = await load_batch_map(session)
    superseded_ids = {by_eid[eid].id for eid in pf["superseded_excluded"] if eid in by_eid}
    repl_id = uuid.UUID(pf["baselines"][REPL]["content_item_id"])
    ...
    sample_item = await repo.get_item(repl_id)
    ...
    # Answer + submit using server-side correct option (not public projection)
    submit_ok = False
    score_ok = False
    ...
    if q1_item and aq_ids:
        ...
        # Save remaining with first option to allow submit
        for cid in aq_ids[1:]:
        ...
        # Post-submit public question with answer reveal path if available
        hist = await asvc.get_question_history(actor.id, aq_ids[0])
        explanation_after = bool(hist) or submit_ok
        # Prefer body explanation presence after submit via review endpoint semantics
        if ver and (ver.body or {}).get("explanation"):
            explanation_after = True

    # Search index for all 100
    search_rows = (
        await session.execute(
            text(
                "SELECT "
    ...
    superseded_search = (
        await session.execute(
            text(
                "SELECT search_text IS NOT NULL, search_vector IS NOT NULL, status "
                "FROM cms.content_items WHERE id = CAST(:id AS uuid)"
            ),
            {"id": str(next(iter(superseded_ids)))},
        )
    ).one()

    return {
        "student_api": {
            "ch02_visible_count": len(bio_visible),
            "expected": 100,
            "superseded_visible_count": len(superseded_visible),
            "R000085_visible": repl_id in visible_ids,
        ...
        "practice_pool": {
            "full_scope_ch02_hits": len(plan_ids & full_pool),
            "chapter_scope_ch02_hits": len(plan_ids & chapter_pool),
            "chapter_id": str(chap_id),
            "superseded_in_pool": len(superseded_ids & full_pool),
            "R000085_in_full_pool": repl_id in full_pool,
            "R000085_in_chapter_pool": repl_id in chapter_pool,
        },
        "practice_now_e2e": {
            ...
            "ch02_selected": bool(aq_ids) and all(uuid.UUID(str(i)) in plan_ids for i in aq_ids),
        ...
        "reindex": {
            ...
            "superseded_status": superseded_search[2],
            "superseded_has_search_vector": bool(superseded_search[1]),
...
def write_report(payload: dict) -> None:
    ...
    lines = [
        ...
        "",
        "PUBLICATION COMPLETE." if str(payload.get("verdict", "")).startswith("GREEN") and payload.get("published_count") == 100 else "",
        "100/100 PUBLISHED." if payload.get("published_count") == 100 else "",
        "STUDENT VISIBILITY VERIFIED." if str(payload.get("verdict", "")).startswith("GREEN") else "",
...
def write_stage(payload: dict) -> None:
    ...
    stage = {
        ...
        "verdict": "GREEN — BIO11-CH02-B001 PUBLISHED AND VERIFIED",
        ...
        "counts": (payload.get("after_snapshot") or {}).get("ch02"),
        "verification_levels": payload.get("verification_levels"),
        "stages": {
            "ncert_certification": "GREEN",
            "publication": "GREEN",
        },
        "artifacts": {"publication_execution_report": str(OUT_JSON)},
    }
    STAGE_JSON.write_text(json.dumps(stage, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    STAGE_MD.write_text(
        f"""# Pipeline stage status — `{BATCH}`

## GREEN — BIO11-CH02-B001 PUBLISHED AND VERIFIED
...
async def run(*, do_commit: bool) -> dict:
    ...
    commands = [
        f"python scripts/execute_bio_ch2_publication.py{' --commit' if do_commit else ''}",
    ]
    auth = (
        "Explicit owner authorization for controlled BIO11-CH02-B001 publication; "
        "publish only the 100 active APPROVED+SOURCE_TEXT_VERIFIED plan IDs via "
        "ContentWorkflowService.publish (CH01 pattern)."
    )
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not do_commit:
            payload = {
                ...
                "bulk_api_used": False,
                "published_count": 0,
                "preflight": {"eligible": 100, "ineligible": 0, "publication_actions": 100},
        ...
        try:
            ...
            payload = {
                ...
                "failure": failure,
                "rollback": "NOT_ATTEMPTED — publish is non-atomic (CH01 pattern)",
                "exact_commands": commands,
            }
            write_report(payload)
            return payload
        ...
        sample_id = uuid.UUID(pf["baselines"][REPL]["content_item_id"])
        ...
        green = (
            ...
            and post["ok"]
            and post["post"]["ch02"] == EXPECTED_CH02_POST
            and student["student_api"]["ch02_visible_count"] == 100
            and student["student_api"]["superseded_visible_count"] == 0
            and student["student_api"]["R000085_visible"]
            and student["practice_pool"]["full_scope_ch02_hits"] == 100
            and student["practice_pool"]["chapter_scope_ch02_hits"] == 100
            and student["practice_now_e2e"]["ch02_selected"]
            ...
            and idem["ok"]
            and tests["ok"]
            and post["post"]["ch01"].get("PUBLISHED") == 100
        ...
        payload = {
            ...
            "publication_plan": pf["plan_actions"],
            "superseded_excluded": pf["superseded_excluded"],
            "r000085": {**pf["r000085"], "published": True, "student_visible": True},
            "preflight": {
                "eligible": 100,
                "ineligible": 0,
            ...
            "exact_commands": commands,
            "assertions": {
                "PUBLISHED_100": post["post"]["ch02"]["PUBLISHED"] == 100,
                "APPROVED_0": post["post"]["ch02"]["APPROVED"] == 0,
                "SUPERSEDED_1": post["post"]["ch02"]["SUPERSEDED"] == 1,
                "SOURCE_TEXT_VERIFIED_100": post["verification_levels"].get("SOURCE_TEXT_VERIFIED")
                == 100,
                "student_visible_100": student["student_api"]["ch02_visible_count"] == 100,
                "practice_pool_100": student["practice_pool"]["full_scope_ch02_hits"] == 100,
                "Q000085_not_visible": student["student_api"]["superseded_visible_count"] == 0,
                "R000085_published_visible": student["student_api"]["R000085_visible"],
                "ch01_unchanged": post["post"]["ch01"].get("PUBLISHED") == 100,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Authorized Biology Ch2 publication")

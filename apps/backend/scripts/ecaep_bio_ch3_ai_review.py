"""READ-ONLY combined ECAEP AI review audit + adjudication for Biology Ch3.
...
Usage:
  python scripts/ecaep_bio_ch3_ai_review.py
"""
...
AUDIT = ROOT / "scientific_ncert_audit.json"
AUDIT_SECOND = ROOT / "second_pass_scientific_ncert_audit.json"
...
def is_batch_item(item: ContentItem) -> bool:
    ...
    if BATCH in tags or any(BATCH in (t or "") for t in tags):
        return True
    return bool(item.slug and "bio11-ch03-b001" in (item.slug or "").lower())
...
def is_ch02_item(item: ContentItem) -> bool:
    tags = item.tags or []
    if CH02 in tags or any(CH02 in (t or "") for t in tags):
        return True
...
async def snap(session) -> dict:
    ...
    ch02 = [i for i in items if is_ch02_item(i)]
    ...
    c2 = Counter(i.status for i in ch02)
...
_NCERT_NORM = {
    "NCERT_DIRECT": "DIRECT",
    "NCERT_SUPPORTED_INFERENCE": "SUPPORTED_INFERENCE",
    "DIRECT": "DIRECT",
...
def load_prior_audit() -> dict[str, dict]:
    """Prefer second_pass_scientific_ncert_audit.json (authoritative for imported CH03).

    Falls back to repair_results.second_pass_audit.results, then first-pass scientific audit.
    Normalizes NCERT_DIRECT → DIRECT for classifier ncert_ok checks.
    """
    results: list[dict] = []
    if AUDIT_SECOND.is_file():
        results = json.loads(AUDIT_SECOND.read_text(encoding="utf-8")).get("results") or []
    elif REPAIR_RESULTS.is_file():
    ...
    if not results and AUDIT.is_file():
        results = json.loads(AUDIT.read_text(encoding="utf-8")).get("results") or []
    out: dict[str, dict] = {}
    for r in results:
        row = dict(r)
        ne = row.get("ncert_evidence")
        if ne in _NCERT_NORM:
            row["ncert_evidence"] = _NCERT_NORM[ne]
        out[row["external_question_id"]] = row
    return out
...
async def main() -> int:
    ...
    safety_ok = (
        ...
        and after["ch02_regression"] == EXPECTED_CH02
        and db_unchanged
    )
    ...
    if not selection_ok or reports_completed != 100 or reports_valid != 100 or not safety_ok:
        ...
        and held_ids == ["GEMINI-20260912-BIO11-CH03-B001-000085"]
        and all(
    ...
    payload = {
        ...
        "assertions": {
            ...
            "CH02 REGRESSION UNCHANGED": after["ch02_regression"] == EXPECTED_CH02,
        },
    }
    OUT_JSON.write_text(json.dumps(payload, indent=2, ensure_ascii=False, default=str) + "\n", encoding="utf-8")

    # Required separate adjudication artifacts (CH01-style companion reports)
    adj_json = ROOT / "ecaep_ai_review_adjudication.json"
    adj_md = ROOT / "ecaep_ai_review_adjudication.md"
    adj_payload = {
        "batch_id": BATCH,
        "generated_at": payload["generated_at"],
        "source_ai_review_report": str(OUT_JSON),
        "ncert_source_of_truth": {
            "path": "StudyMaterial/Biology/Class 11-Biology/ncert-books-class-11-biology-chapter-3.pdf",
            "sha256": "7fc8d9e8416e33ec974c288099e9fb7739d64f40b9b87bcdaf1f428f27d472cc",
            "extract": str(ROOT / "_source_extract.txt"),
        },
        "verdict": verdict,
        "summary": {
            "total": len(records),
            "approval_ready": approval_content_ready,
            "held": len(held),
            "readiness_counts": dict(adj_counts),
        },
        "adjudications": [
            {
                "question_id": r["question_id"],
                "ai_review_verdict": r["ai_review_verdict"],
                "findings": {
                    "flags": (r.get("findings") or {}).get("flags"),
                    "reason": (r.get("findings") or {}).get("reason"),
                    "confidence": (r.get("findings") or {}).get("confidence"),
                },
                "adjudication": r["adjudication"],
                "approval_eligibility": r.get("approval_eligibility"),
            }
            for r in records
        ],
        "held_question_ids": held_ids,
        "repair_authorized": False,
        "content_mutations": 0,
    }
    adj_json.write_text(json.dumps(adj_payload, indent=2, ensure_ascii=False, default=str) + "\n", encoding="utf-8")
    adj_md.write_text(
        f"""# ECAEP AI Review Adjudication — `{BATCH}`

## Verdict

**{verdict}**

Source of truth: NCERT Class 11 Biology Ch3 Plant Kingdom  
PDF SHA-256: `7fc8d9e8416e33ec974c288099e9fb7739d64f40b9b87bcdaf1f428f27d472cc`

## Summary

- Total adjudicated: **{len(records)}**
- Approval-ready: **{approval_content_ready}**
- Held (not approval-ready): **{len(held)}**
- Readiness counts: `{dict(adj_counts)}`
- Content repair during this stage: **NOT AUTHORIZED / NOT EXECUTED**

## Held questions

{chr(10).join(f'- `{h}`' for h in held_ids) if held_ids else '- (none)'}

## Notes

Adjudications are derived from live `ai_check_report` plus prior second-pass scientific/NCERT audit and NCERT extract cross-checks. WARNING/DISCREPANCY findings are classified VALID_FINDING vs FALSE_POSITIVE via the CH02-pattern adjudicator; genuine defects are held without repair.
""",
        encoding="utf-8",
        newline="\n",
    )

...
SYSTEM_PROMPT = """You compile a one-page NEET revision sheet grounded in verified structured \
facts extracted from real NCERT textbook sections covering an entire chapter. Given facts \
from multiple sections, produce exactly one sheet as strict JSON, nothing else — no markdown \
...
the given facts — do not introduce formulas or facts they don't support."""


def build_prompt(*, chapter_name: str, excerpts: list[tuple[str, str]]) -> str:
    lines = [f"Chapter: {chapter_name}", "", "Verified facts by section:"]
